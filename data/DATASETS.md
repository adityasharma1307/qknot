...
union of two snapshots rather than one census. `scripts/trim_head_stratum.py`
...
`scripts/sample_longtail.py`: membership of "the top 10,000" is only well
...
```
python scripts/sample_longtail.py --seed 20260725 --head-ids data/head_10k_2026-07-25.jsonl
...
`scripts/redteam_check.py` re-derives the published sample from it and confirms
...
`scripts/redteam_check.py` re-checks every claim above: sample integrity,
