...
`release/qknot-0.1.0.bundle.json` and `release/README.md`.

**Identity registration.** `qknot register` binds a post-quantum key to your
...
`qknot verify --registration` gives the composed verdict: not just "the
signature is valid" but "and it's attributable to this identity, on this
basis." `qknot trust-material` fetches a real Fulcio/Rekor trust store from
...
**Audit.** `qknot scan` / `scan-ids` / `audit-npm` / `audit-pypi` /
...
`qknot trust-material`, the identity-registration product surface
