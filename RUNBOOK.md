...
the script as
`powershell -ExecutionPolicy Bypass -File scripts\run_20k_audit.ps1 -Step estimate`.
...
reason in `data\DATASETS.md` — an undated filename is exactly what let the July
...
```powershell
python stats.py --head data\head_10k_<date>.jsonl `
