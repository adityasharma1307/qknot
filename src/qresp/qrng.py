...
COMMITMENT_DOMAIN = b"qresp-entropy-attestation-v1"
...
class AnuQrngBackend:
    ...
    def _request_block(self, session, count: int) -> bytes:
        params = {"length": count, "type": "uint8"}
        headers = {"User-Agent": "qresp/0.2 (BITS Pilani CS F376 research)"}
        if self.api_key:
...
def get_backend(name: str, **kwargs) -> EntropyBackend:
...
def get_entropy(
    ...
    """Acquire `n_bytes` of entropy and attest to its origin.
